                ...
                @foreach ($folders as $folder)
                    <li x-data="{ open: false }" class="">
                        <div @click="open = !open" class="cursor-pointer flex items-center space-x-2">
                            ...
                            <strong class="text-lg  hover:underline ml-1"> {{ $folder->nom }}</strong>
                        </div>
                        <ul x-show="open"
                            class="list-inside pl-5 mt-2 transition-all duration-300 ease-in-out overflow-hidden">
                            @foreach ($folder->standards as $standard)
                                <li class="text-gray-700 dark:text-gray-300 pl-8 flex border-l"> <x-icons.pdf
                                        class="w-6 h-6" /><a href="{{ $standard->path }}" class="lien">
                                        {{ $standard->nom }}
                                    </a>
                                </li>
                            @endforeach
