            ...
            <div class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                ...
                <div class="flex items-center lg:flex-col">
                    ...
                    @if (!$notification->read)
                        <button type="button" class="btn-select-bottom-right"
                            onclick="marquerCommeLu({{ $notification->id }})" title="Marquer comme Lu">
                            <x-icon type="read" size="1" class="icons-no_hover" />
                        </button>
                    @else
                        <button type="button" class="btn-select-bottom-right"
                            onclick="marquerCommeNonLu({{ $notification->id }})" title="Marquer comme non-lu">
