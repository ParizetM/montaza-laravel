...
<tbody>
    @if (!$isSmall && !$isMid && isset($cdesGrouped) && $cdesGrouped->count() > 0)
                            ...
                            @if ($total > 0)
                                <x-tooltip position="top">
                                    <x-slot name="slot_item">
                                        <x-icon :size="1" type="error_icon"
                                            class="fill-red-500 dark:fill-red-400" />
                                    </x-slot>
                                    <x-slot name="slot_tooltip">
                                        <span class="text-red-600 dark:text-red-400 font-semibold">
                                            Les stocks à prendre en compte pour cette commande n'ont pas encore été
                                            précisés.
                                        </span>
                                    </x-slot>
                                </x-tooltip>
                            ...
                            <x-tooltip position="right">
                                ...
                                <x-slot name="slot_tooltip">
                                    <div class="flex flex-col max-w-md">
                                            ...
                                            <table class="min-w-full">
                                                ...
                                                <tfoot>
                                                    <tr class="font-bold bg-gray-50 dark:bg-gray-800">
                                                        <td colspan="3" class="px-2 py-1 text-xs text-right">Total:
                                                        </td>
                                                        <td class="px-2 py-1 text-xs text-right whitespace-nowrap">
                                        ...
                                        @else
                                            <p class="text-gray-600 dark:text-gray-300">Aucune ligne dans cette commande
                                            </p>
                                        @endif
                                    </div>
                                </x-slot>
        ...
        @endforeach
    @else
        <!-- Affichage standard sans groupement -->
        @foreach ($cdes as $cde)
            <tr class="border-b border-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 dark:border-gray-700 cursor-pointer"
                onclick="window.location='{{ route('cde.show', $cde) }}'">
                                    ...
                                    @if ($cde->cdeLignes->count() > 0)
                                        <table class="min-w-full">
                                            ...
                                            <tfoot>
                                                ...
                                                <tr class="font-bold bg-gray-50 dark:bg-gray-800">
                                                    <td colspan="3" class="px-2 py-1 text-xs text-right">Total TTC:
                                                    </td>
                                                    <td class="px-2 py-1 text-xs text-right whitespace-nowrap">
                                                        {{ formatNumberArgent($cde->total_ttc) }}
                                                    </td>
