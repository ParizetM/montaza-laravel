<div class="bg-white dark:bg-gray-800 flex flex-col p-4 text-gray-800 dark:text-gray-200 h-full">
    <div class="flex justify-between items-center">
        <h1 class="text-3xl font-bold mb-1">
            <a href="{{ route('affaires.index') }}" class="hover:bg-gray-100 dark:hover:bg-gray-700 p-1 rounded-sm">
                {{ __('Affaires') }}
            </a>
        </h1>
        <div>
            <a href="{{ route('affaires.index') }}" class="btn mb-2">Voir tout</a>
        </div>
    </div>
    ...
    <table class="w-full text-left border-collapse">
        ...
        <tbody>
            @foreach ($affaires as $affaire)
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer" onclick="window.location='{{ route('affaires.show', $affaire) }}'">
