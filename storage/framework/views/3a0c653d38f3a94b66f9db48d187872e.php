<div class="bg-white dark:bg-gray-800 flex flex-col h-full rounded-xl overflow-hidden shadow-lg border border-amber-100 dark:border-amber-900/50 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl">
    ...
    <div class="px-3 py-2 border-b border-amber-100 dark:border-amber-900/50 flex justify-between items-center bg-amber-600 dark:bg-amber-900/20">
        <div class="flex items-center gap-2">
            ...
            <h2 class="font-bold text-sm text-white dark:text-amber-100">
                <a href="<?php echo e(route('devis_tuyauterie.index')); ?>" class="hover:underline transition"><?php echo e(__('Devis')); ?></a>
            </h2>
        </div>
        <a href="<?php echo e(route('devis_tuyauterie.index')); ?>" class="text-[10px] font-bold text-amber-100 hover:text-white dark:text-amber-400 dark:hover:text-amber-300 transition-colors uppercase tracking-wider">
            Voir tout
        </a>
    </div>
    ...
    <div class="flex-1 overflow-y-auto p-2 scrollbar-thin scrollbar-thumb-amber-200 dark:scrollbar-thumb-amber-800">
         <?php if(!$isSmall): ?>
            <div class="mb-4 px-2 pt-2">
                <?php if (Auth::check() && Auth::user()->hasPermission('gerer_les_devis')): ?>
                    <a href="<?php echo e(route('devis_tuyauterie.create')); ?>" class="w-full flex justify-center items-center gap-2 px-4 py-2 bg-amber-600 text-white text-xs font-semibold rounded hover:bg-amber-700 transition shadow-sm group">
        ...
        <?php if($devis->count() > 0): ?>
            <div class="space-y-1">
                <?php $__currentLoopData = $devis->take(5); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="p-2 hover:bg-amber-100 dark:hover:bg-amber-900/20 rounded-md transition-all duration-300 hover:scale-[1.03] hover:shadow-lg group cursor-pointer border border-transparent hover:border-amber-200 dark:hover:border-amber-800/30" onclick="window.location='<?php echo e(route('devis_tuyauterie.show', $item->id)); ?>'">
                        <div class="flex justify-between items-start mb-0.5">
                            <span class="font-bold text-xs text-gray-800 dark:text-gray-200 group-hover:text-amber-700 dark:group-hover:text-amber-300 transition-colors">
                             ...
                             <span class="text-[9px] text-gray-400 font-mono">
                                <?php echo e($item->date_emission ? \Carbon\Carbon::parse($item->date_emission)->format('d/m') : '-'); ?>

                            </span>
                        </div>
                        <div class="text-[11px] text-gray-500 dark:text-gray-400 line-clamp-1 mb-1.5 font-medium leading-tight">
                            <?php echo e($item->client_nom); ?>
...
<?php /**PATH /home/vagrant/code/montaza/resources/views/devis_tuyauterie/index_col.blade.php ENDPATH**/ ?>